...
parser = argparse.ArgumentParser(
    description='Code for detecting peeled areas in vegetables')
parser.add_argument('--folder',
                    help='Path to input images.',
                    default="wet_regions")
parser.add_argument(
    '--image',
    help='Name of image to process. If empty all images are processed.',
    default='')
args = parser.parse_args()

# Iterate through folder or select specific image
path = os.path.join("./img_data/", args.folder)
images = next(os.walk(path),
              (None, None, []))[2] if args.image == '' else [args.image]
...
for imFile in sorted(images):
    ...
    morph1 = morph(redThresh1)
    morph2 = morph(redThresh2)
    morph3 = morph(redThresh3)
    ...
    def findDrawFirstContour(imgDraw, imgBinary):
        # Get mask
        contours, hierarchy = cv2.findContours(np.invert(imgBinary),
                                               cv2.RETR_TREE,
    ...
    def findDrawOverlap(imgDraw, vegMask, imgThresh):
        ...
        contours, hierarchy = cv2.findContours(overlap, cv2.RETR_TREE,
                                               cv2.CHAIN_APPROX_SIMPLE)
        print(f'Number of contours: {len(contours)}')
    ...
    # Display
    titles = [
        'Red - Global Threshold', 'Red - Otsu', 'Red - Otsu + Threshold',
        'Global Threshold - Morphology', 'Otsu - Morphology',
        'Otsu + Threshold - Morphology', 'Vegetable Outline',
        'Vegetable Outline', 'Vegetable Outline', 'Vegetable Mask',
        'Vegetable Mask', 'Vegetable Mask', 'Peeled Mask', 'Peeled Mask',
        'Peeled Mask'
    ]

    images = [
        redThresh1, redThresh2, redThresh3, morph1, morph2, morph3, imgDraw1,
        imgDraw2, imgDraw3, vegMask1, vegMask2, vegMask3, peelMask1, peelMask2,
        peelMask3
    ]
